jQuery(document).ready(function ($) {
    ...
    console.log("kind_customer=" + kind_customer)
    ...
    function getallorders() {
        ...
        if ($('.date_time_with_filter').length > 0) {
            json_data.push($('.date_time_with_filter').val().split('T')[0]);
        }
        // console.log(json_data) 
        let dataString = 'action=GetProblems&value=' + JSON.stringify(json_data) + '&value1=2&value2=0&date_alarm_from=' + date_alarm_from + '&date_alarm_to=' + date_alarm_to;
    ...
    function init_table(value_table) {
        ...
        $('.save_statuse_service').off().on('click', function () {
            ...
            $.ajax({
                ...
                success: function (html) {
                    $('.main_p_1_service').css('background-color', $('#serve_customer_' + serviceValue).data('color'));
                    $('.main_p_1_service').text($('#serve_customer_' + serviceValue).data('name'));

                    $('.main_p_2_service').css('background-color', $('#table_stars_' + statusValue).data('color'));
                    $('.main_p_2_service').text($('#table_stars_' + statusValue).data('name'));
                }
        ...
        $('body').on('click', '.show_statuse', function () {
            var td   = $(this);
            var id   = $(this).attr('id');
            var data = {
                '1': 'لم يتم التحديد',
                    ...
                    inputValidator: (value) => {
                        if (!value) {
                            ...
                            $.ajax({
                                ...
                                success: function (html) {
                                    update_tr_row(id);
                                    $('.show_statuse_dialog').css('background-color', $('#table_status_' + value).data('color'));
                                    $('.show_statuse_dialog').text($('#table_status_' + value).data('name'));
                                }
                            }).done(function (msg) {
                                if (msg === "11111") {
                                    Toast.fire({
        ...
        $('.all_orders').find(".edite_order_main").off().on("click", function () {
            var id = $(this).attr("id");
            $('.set_order_alarm_id').attr('id', id);
            $('.add_to_fovreite_profile').attr('id', id);
            $('.add_to_fovreite_profile').attr('is_true', $(this).data('favourite'));
            $('.add_to_motabaa_profile').attr('id', id);
            $('.statuse_order_id').val(id);
            $('.add_to_motabaa_profile').attr('is_true', $(this).data('motabaa'));

            if ($(this).data('motabaa') == 1) {
                $('.add_to_motabaa_profile').addClass('motabaa-button');
            }
            else {
                $('.add_to_motabaa_profile').removeClass('motabaa-button');
            }
            if ($(this).data('favourite') == 1) {
                $('.add_to_fovreite_profile').addClass('favorite-button');
            }
            else {
                $('.add_to_fovreite_profile').removeClass('favorite-button');
            }
        ...
        $('.open_message_notifcation').on('click', function () {
            openEditOrder($(this).val());
        });
        ...
        $('body').on('click', ".add_new_alarm", function () {
            var id = $(this).attr("id");
        ...
        $('body').find(".add_to_fovreite").off().on('click', function () {
            let id      = $(this).attr("id");
        ...
        $('body').find(".add_to_motabaa").off().on('click', function () {
            let id      = $(this).attr("id");
            let is_true = $(this).attr("is_true");
            let object  = $(this);
        ...
        $('.all_orders').find(".change_note").off().on('click', function () {
            ...
            var dataString = 'action=listDetailsLogs&value=' + order_id + '&value1=' + 1;
            $.ajax({
                type: "POST",
                url: url_action,
                data: dataString,
                ...
                $('#show_order_details_logs_body_content').html(response);
                $('#show_order_details_logs').modal('show');
                $('.add_new_note').attr('id', order_id);
            });
        });
                ...
                <div class="form-row" style="margin-top:10px">
                    ...
                    <div class="form-row">
                        ...
                        <select id="inputState" class="form-control">
                            ...
                            <option value="">تم التواصل</option>
                    ...
                    <div class="form-row">
                        ...
                        <textarea class="form-control" id="cusom_description" rows="10" disabled></textarea>
                        </div>  
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-12">
        ...
        $('.save-order-datialse').off().on('click', function () {
            ...
            let hiring_date                  = formm.find('#hiring_date').val();
            let military_rank                = formm.find('#military_rank').val();
            let net_salary                   = formm.find('#net_salary').val();
            let gross_salary                 = formm.find('#gross_salary').val();
            let support_premium              = formm.find('#support_premium').val();
            let remain_money                 = formm.find('#remain_money').val();
            let state_value                  = formm.find('#state_value').val();
            let mortgage_lenders_id          = formm.find('#mortgage_lenders_id').val();
            let construction_completion_rate = formm.find('#construction_completion_rate').val();
            let state_age                    = formm.find('#state_age').val();
            let years_left_repay             = formm.find('#years_left_repay').val();
            let job_title                    = formm.find('#job_title').val();
            ...
            if (cusom_statues == 1) {
                ...
                var dataString = 'action=editeorderdetailse' + '&job_title=' + job_title + '&years_left_repay=' + years_left_repay + '&state_age=' + state_age + '&construction_completion_rate=' + construction_completion_rate + '&mortgage_lenders_id=' + mortgage_lenders_id + '&state_value=' + state_value + '&remain_money=' + remain_money + '&support_premium=' + support_premium + '&gross_salary=' + gross_salary + '&net_salary=' + net_salary + '&military_rank=' + military_rank + '&hiring_date=' + hiring_date + '&value=' + cusom_number + '&value1=' + cusom_name + '&value2=' + cusom_starts + '&value3=' + cusom_statues + '&value4=' + cusom_description + '&value5=' + number_owner + '&value6=' + kind_aqar + '&value7=' + city + '&value8=' + jop + '&value9=' + salary + '&value10=' + resource_obligations + '&value11=' + obligations + '&value12=' + data_birth + '&value13=' + powered + '&value14=' + bank + '&value15=' + name_owner + '&value16=' + order_notes;
                $.ajax({
                    type: "post",
                    url: url_action,
                    data: dataString,
    ...
    function function_files() {
        ...
        pond.on('processfile', (error, file) => {
            ...
            var path       = "https://demo.alwaseet.sa/assets/api/tmp/" + file.serverId + "/" + file_name + "." + file_extension;
    ...
    $('.add_new_order').on('click', function () {
        ...
        var select_source = model.find('.select_source').val();
        var emp_id        = model.find('.select_employee').val();
        ...
        if ($.trim(emp_name) === "") {
            ...
            var dataString = 'action=add_employ_order&value=' + emp_name + '&value1=' + emp_phone + '&note=' + note + '&source=' + select_source + '&emp_id=' + emp_id;
            $.ajax({
                type: "post",
                url: url_action,
                data: dataString,
                ...
                if (msg == "11111") {
                    ...
                    model.find(".close_add_new_customer").click();
                    getallorders();
    ...
    function update_tr_row(order_id) {
        var dataString = 'action=UpdateRowCustomer&value=' + order_id + '&value1=' + 1;
        $.ajax({
            type: "POST",
            url: url_action,
            data: dataString,
    ...
    $('.add_new_note').on('click', function () {
        ...
        changeNote(order_id, new_note);
    });

    function changeNote(id, note) {
        var dataString = 'action=changenote&value=' + id + '&value1=' + note;
        $.ajax({
            type: "post",
            url: url_action,
            data: dataString,
            ...
            let now = new Date();
            $('#show_order_details_logs_body_content').append(`
                <tr>
                    <td>
                        <h5 style="text-align: left"> ${now.getFullYear() + '-' + padNumber(now.getMonth() + 1) + '-' + padNumber(now.getDate()) + ' ' + padNumber(now.getHours()) + ':' + padNumber(now.getMinutes()) + ':' + padNumber(now.getSeconds())} </h5>
                        <h5 style="text-align:right">${$('.user-panel .info p').html()}</h5>
                        <p style="font-weight: 700;">${$('#new_note').val()}</p>
                    </td>
                </tr>
            `);
            $('#new_note').val('');
    ...
    $('body').on('click', '.change_alarm_note', function () {
        let alarm_id   = $(this).attr('id');
        let inputValue = $(this).data('note');
        let order_id   = $(this).data('order');
        Swal.fire({
            title: 'تغير محتوى التذكير',
            input: 'textarea',
            showCancelButton: true, // Show the close button
            confirmButtonText: 'تغير', // Text for the OK button
            cancelButtonText: 'اغلاق', // Text for the close button
            inputAttributes: {
            ...
            customClass: {
                input: 'custom-textarea' // add custom CSS class to the textarea
            }
        }).then(function (result) {
            if (result.value) {
                var dataString = 'action=change_alarm_note&value=' + result.value + '&value1=' + alarm_id;
                $.ajax({
                    type: "post",
                    url: url_action,
                    data: dataString,
    ...
    $('body').on('click', '.show_details_logs', function () {
        let order_id   = $(this).attr('id');
        var dataString = 'action=listDetailsLogs&value=' + order_id + '&value1=' + 1;
        $.ajax({
            type: "POST",
    ...
    $('body').on('click', '.whatsapp_send', function () {
        let phone_number  = $(this).data('phone');
        let order_id      = $(this).data('order');
        let customer_name = $(this).data('name');
        $('#whatsapp_phone_number').val(phone_number);
        var dataString = 'action=listWhatsappTemplate&order_id=' + order_id + '&customer_name=' + customer_name;
        $.ajax({
            type: "POST",
            url: url_action,
            data: dataString,
            ...
            $('.listWhatsappTemplate_content').html(response);
            $('#whatsapp_template').modal('show');
        });
    });

    $('body').on('click', '.go-whatsapp', function () {
        let content          = $(this).parent().parent().find('.templete').val();
        let number           = '966' + $('#whatsapp_phone_number').val();
        window.location.href = 'https://api.whatsapp.com/send?phone=' + number + '&text=' + content;
    });
